...
import { fetchFolders } from '../../../api/library/fetchFolders';
...
const FolderSelectModal = ({ isOpen, onClose, libraryId, isDownloadContain }) => {
  ...
  const handleFolderClick = async (folderId) => {
    ...
    try {
        ...
        // Use a callback function to ensure the navigation happens after state update
        setNoteId(data => {
            onClose();
            navigate(`/note-editor?folderId=${folderId}&noteId=${data}`);
            return data; 
        });
    } catch (error) {
        console.error('폴더 다운로드 중 오류가 발생했습니다.');
    }
    }
